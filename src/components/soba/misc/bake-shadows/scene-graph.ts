...
@Component({
	...
	template: `
		...
		<!-- Spinning box -->
		<ngt-mesh #box [castShadow]="true" [position]="[0, 1, 0]">
			<ngt-box-geometry *args="[1, 1, 1]" />
			<ngt-mesh-standard-material color="#4ecdc4" />
		</ngt-mesh>

		<!-- Spinning torus -->
		<ngt-mesh #torus [castShadow]="true" [position]="[1.5, 1.2, 0]">
	...
	imports: [NgtArgs, NgtsBakeShadows, NgtsOrbitControls],
})
export class SceneGraph {
	...
	private boxRef = viewChild<ElementRef<THREE.Mesh>>('box');
	private torusRef = viewChild<ElementRef<THREE.Mesh>>('torus');
	...
	constructor() {
		...
		beforeRender(({ delta }) => {
			...
			const torus = this.torusRef()?.nativeElement;
			if (torus) {
