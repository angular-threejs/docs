...
export class Torus {
  ...
  protected color = computed(() =>
    this.hovered() ? "mediumpurple" : "orange",
  );
}
...
export class FaceMaterial {
  ...
  protected color = computed(() =>
    this.box.isHovered() === this.index() ? "mediumpurple" : "orange",
  );
}
...
@Component({
  ...
  template: `
    <ngt-mesh
      ...
      (pointermove)="
        $event.stopPropagation(); hovered.set($event.face.materialIndex)
      "
    ...
    <ngt-portal [container]="scene()" autoRender>
      <ng-template portalContent>
        ...
        <ngts-perspective-camera
          [options]="{ makeDefault: true, position: [0, 0, 10] }"
        />
