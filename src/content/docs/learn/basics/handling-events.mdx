...
Additionally, there are two special events `(updated)` and `(attached)` that Angular Three has for all elements. [Events reference page](/reference/core/primitive) also mentions these two events.
