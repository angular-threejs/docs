...
import FirstScene from "../../../../components/scenes/first-scene/first-scene";
...
First step is to open up the `app.component.ts` file and import [`NgtCanvas`](/reference/core/dom/canvas-tbd) component then render it on the template.
...
Notice that `NgtCanvas` is imported from [`angular-three/dom`](/reference/core/dom-tbd). This allows `angular-three` to target the appropriate platform that your application is running on.
...
`NgtCanvas` also provides a [**store**](/reference/core/store-tbd) that the Angular Three application will act upon. To improve accessing this store via [Dependency Injection](https://angular.dev/guide/dependency-injection), it is [recommended](/learn/basics/app-structure) to create a separate component `scene-graph.ts` and render it as the `NgtCanvas` content.
...
But before we can do so, we need to tell Angular Three about these objects by [extending the catalogue](/reference/core/extend-tbd). By default, this catalogue is empty. Angular Three maps the catalogue to Custom Elements tags with the following naming convention:
...
Internally, Angular Three uses an attribute [`attach`](/reference/core/attach-tbd) to **attach an object to a property of its parent**
...
As for entities like `THREE.BoxGeometry` whose **Constructor Arguments** changes will require the entity to be **reconstructed**, Angular Three provides a structural directive called [`NgtArgs`](/reference/core/args-tbd). Let's see how we can use it to modify the `THREE.BoxGeometry`
...
You can learn more about how Angular Three handles THREE.js properties via **Property Binding** [here](/reference/core/renderer-tbd)
