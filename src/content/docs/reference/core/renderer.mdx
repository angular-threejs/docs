...
You can pass a dynamic value by using **Property Binding** syntax `[attach]`. When this is the case, `attach` accepts `Array<string | number>` in addition to just `string`. Common use-case for this is to attach difference materials to difference faces of a geometry.
