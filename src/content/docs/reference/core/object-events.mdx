...
This is intended to be used as a [`hostDirectives`](https://angular.dev/guide/directives/directive-composition-api) on a custom abstraction. This is useful for cases where you don't know which events the abstraction's consumers will use. `NgtObjectEvents` calls `injectObjectEvents` internally.
