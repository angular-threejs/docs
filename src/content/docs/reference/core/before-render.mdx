...
`injectBeforeRender` is a [Custom Inject Function](https://nartc.me/blog/inject-function-the-right-way/) that allows you to execute logic before each frame render. This is useful for animations, updates, or any computations that need to happen on every frame.
...
Since `injectBeforeRender` is a CIF, it has to be invoked in an [**Injection Context**](https://angular.dev/guide/di/dependency-injection-context). In order to invoke `injectBeforeRender` outside of an **Injection Context**, you can provide an `Injector` to the `injector` property of the 2nd argument to `injectBeforeRender`
