...
@Component({
  ...
  template: `
    <!-- Sun mesh - this is the light source -->
    <ngt-mesh #sunMesh [position]="[5, 3, -15]">
      <ngt-sphere-geometry *args="[2, 32, 32]" />
      <ngt-mesh-basic-material color="#ffaa00" />
    </ngt-mesh>

    <!-- Scene objects that will cast shadows in the rays -->
    <ngt-mesh [position]="[0, 0, 0]">
      <ngt-box-geometry *args="[2, 4, 0.5]" />
      <ngt-mesh-standard-material color="#333" />
    </ngt-mesh>
...
- The sun mesh should use `MeshBasicMaterial` for best results (it should be bright/emissive)
...
- Works best when the sun is partially visible or at the edge of the frame
